...
2. **495 次回收的原因是 humongous 分配** —— 这是 GC 日志说的话。G1 下超过 region 一半的对象直接
   进老年代;工作集加载完之后的整场风暴里,`Humongous regions` 稳定在 197~234、从不回落(出现最多的
   就是 `200->200`,516 次),也就是常年约 200 MB 以巨型区域的形式占着堆。(这个原因数以前印的是
   1894 —— 而 1894 恰好就是整份日志里出现 `Humongous regions:` 这一行的次数:旧匹配器把记账行
   当成了原因。记账不是原因。)
